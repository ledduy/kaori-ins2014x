...
/*
 * 		The config file is used for feature extraction.
 * 		
 * 		Using different config files, different feature types are generated for experiments. 
 */
...
function generateCHFeatureConfigFile($szFPOutputFN, $szColorType="CV_HSV", $nNumRows=5, $nNumCols=5,
$nNumBins1=4, $nNumBins2=4, $nNumBins3=4)
{

	$arOutput = array();
	$arOutput[] = sprintf("-desc dvf - color histogram, %dx%d grid, %s, histograms (0, 255, %d) - (0, 255, %d) - (0, 255, %d)",
	$nNumRows, $nNumCols, $szColorType, $nNumBins1, $nNumBins2, $nNumBins3);
...
function generateCMFeatureConfigFile($szFPOutputFN, $szColorType="CV_Luv", $nNumRows=5, $nNumCols=5)
{
	...
	$arOutput = array();
	$arOutput[] = sprintf("-desc dvf - color moments, %dx%d grid, %s",
	$nNumRows, $nNumCols, $szColorType);
...
function generateEOHFeatureConfigFile($szFPOutputFN, $nNumRows=5, $nNumCols=5, $nNumEdgeBins=10, $tLow = 10.0, $tHigh = 30.0)
{
	...
	$arOutput = array();
	$arOutput[] = sprintf("-desc dvf - edge orientation histogram, %dx%d grid, CV_BGR2GRAY, histogram (0, 360, %d+1), thresholds (%0.2f, %0.2f)",
	$nNumRows, $nNumCols, $nNumEdgeBins, $tLow, $tHigh);
...
function generateLBPFeatureConfigFile($szFPOutputFN, $nNumRows=5, $nNumCols=5, $nNumBins=10)
{
	...
	$arOutput = array();
	$arOutput[] = sprintf("-desc dvf - local binary pattern, %dx%d grid, CV_BGR2GRAY, histogram (0, 59, %d)",
	$nNumRows, $nNumCols, $nNumBins);
