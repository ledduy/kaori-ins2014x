...
function rerank_using_RANSAC_fgbg_tfidf_avg(data_name, base_feature, base_RANSAC, start_video_id, end_video_id)
% Example:
...
RESULT_RUN_ID = ['R0_', data_name, '.', base_feature, '+RANSAC_fg+bg_tfidf_avg'];
...
for q_id = start_query_id:end_query_id	% Duyet qua tat ca cac cau query
	...
	for id = start_video_id:end_video_id
		...
		for shot_idx = 1:nshot	% duyet qua tat ca cac shot trong danh sach cua RANSAC
			...
			for frame_idx=1:nframe % duyet qua tat ca cac frame ma co su dung DPM
				...
				if ~isempty(inliers_struct.bg_inlier_loc{shot_idx}{frame_idx})
					...
					for ii = 1:nquery
						if size(inliers_struct.bg_inlier_loc{shot_idx}{frame_idx}{ii},2) < 4
							inliers_struct.bg_inlier_loc{shot_idx}{frame_idx}{ii} = [];
						end
					end
		...
		clear inliers_struct fg_locs fg_kp bg_kp frame_names shot_id
	end
end
