...
RESULT_RUN_ID = ['CVPR_R0_', data_name, '.', base_BOW_fusion, '+DPM+woRANSAC_old'];

if is_using_RANSAC
	RESULT_RUN_ID = ['CVPR_R0_', data_name, '.', base_BOW_fusion, '+DPM+RANSAC_ghi_log_Ndeq0_Nwgt5'];
end
...
if is_using_RANSAC
	...
	for q_id = start_query_id:end_query_id	% Duyet qua tat ca cac cau query
		...
		for id = start_video_id:end_video_id
			...
			dpm_ransac_log_file = fullfile(final_result_dir, ['/TRECVID2013_', num2str(id),'.log']);
			...
			dpm_ransac_log_local_file = fullfile(final_result_local_dir, ['/TRECVID2013_', num2str(id),'.log']);
			if ~debug_mode
				fid = fopen(dpm_ransac_res_local_file, 'w');
				log_fid = fopen(dpm_ransac_log_local_file, 'w');
			end
			...
			for shot_idx = 1:nshot	% duyet qua tat ca cac shot trong danh sach cua RANSAC
				...
				for frame_idx=1:length(frame_locs) % duyet qua tat ca cac frame ma co su dung DPM
					...
					%new_scores(end+1) = max(Nd+N_fg, 1)*log2(max(2,N_bg))*P_score; 	% formula24 
					...
					if ~debug_mode && Nd == 0 && N_fg-Nd > 4
						fprintf(log_fid, '%s #$# %s #$# %d #$# %d #$# %d\n', shot, frame_name{end}{1}, Nd, N_fg-Nd, N_bg); % formula21: (max+mean)/2
					end
					if debug_mode
			...
			if ~debug_mode
				fclose(fid);
				fclose(log_fid);
				status = unix(['mv ' dpm_ransac_log_local_file ' ' dpm_ransac_log_file]);
