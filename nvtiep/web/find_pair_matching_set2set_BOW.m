function [nshares_fg, nshares_bg] = find_pair_matching_set2set_BOW(data_name, test_pat, query_pat, output_file, runID, qr_shotID, db_shotID, output_dir)
...
for i = 1:length(lst_qr_frame_name)
	for j=1:sampling_rate:length(lst_db_frame_name)
		...
		[score, new_output_img, nfg, nbg] = find_pair_matching_BOW(data_name, test_pat, query_pat, lst_qr_frame_name{i}, query_index, i, db_img, j, output_image, runID, frame_quant_info, query_filenames, topic_bows, bins, clip_frame, clip_kp);
		%find_pair_matching(lst_qr_frame_name{i}, db_img, output_image, runID)
